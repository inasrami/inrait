          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="font-semibold text-[15px] text-black leading-tight">
                -{{ currentDiscount.percent }}% {{ t('servicesPage.bannerLabel') }}
              </div>
      ...
      <div v-if="hasSelection" class="quote-bar">
        <div class="max-w-[1080px] mx-auto px-6 flex items-center justify-between flex-wrap gap-4">
          ...
          <div class="flex items-center gap-4">
            ...
            <RouterLink
              :to="{ path: '/contact', query: { services: selectedServices.join(','), addons: selectedAddons.join(',') } }"
              class="btn-primary text-[13px] py-3 px-6"
              style="cursor: pointer;"
            >
...
function serviceById(id)     { return SERVICES.value.find(s => s.id === id) }
...
function toggleAddon(addonId, serviceId) {
  // Auto-select parent service if not already selected
...
const subtotal = computed(() => {
  ...
  selectedServices.value.forEach(id => {
    total += serviceById(id)?.basePrice ?? 0
  })
...
.banner-pct {
  font-size: 11px; font-weight: 800; color: rgba(0,0,0,0.7);
  background: rgba(0,0,0,0.1); border-radius: 100px;
  padding: 3px 8px; letter-spacing: 0.04em;
}
.banner-enter-active, .banner-leave-active { transition: all 0.35s cubic-bezier(0.16,1,0.3,1); }
...
.selected-badge {
  ...
  color: #000; letter-spacing: 0.02em; pointer-events: none;
}
.badge-enter-active, .badge-leave-active { transition: opacity 0.2s ease, transform 0.2s ease; }
...
.select-toggle--on { background: var(--accent); border-color: var(--accent); }
...
.addons-drawer {
  padding: 18px 22px 20px; background: rgba(255,255,255,0.018);
...
.addon-row--checked { background: rgba(164,224,75,0.05); border-color: rgba(164,224,75,0.2); }
.addon-check {
...
.addon-check--on { background: var(--accent); border-color: var(--accent); }
.addon-label { flex: 1; font-size: 13px; color: var(--text-muted); transition: color 0.18s ease; }
.addon-row--checked .addon-label { color: var(--text); }
.addon-price { font-size: 12px; font-weight: 600; color: var(--accent); flex-shrink: 0; }

.drawer-enter-active, .drawer-leave-active {
  transition: opacity 0.25s ease, max-height 0.3s cubic-bezier(0.16,1,0.3,1);
  overflow: hidden; max-height: 500px;
}
.drawer-enter-from, .drawer-leave-to { opacity: 0; max-height: 0 !important; }
...
.quote-bar {
  ...
  -webkit-backdrop-filter: blur(24px); border-top: 1px solid var(--border-strong);
  padding: 14px 0;
}
.quotebar-enter-active, .quotebar-leave-active { transition: all 0.4s cubic-bezier(0.16,1,0.3,1); }
.quotebar-enter-from, .quotebar-leave-to { opacity: 0; transform: translateY(100%); }

.summary-chip {
  display: inline-flex; align-items: center; gap: 6px; font-size: 12px;
  font-weight: 500; color: var(--text-muted); background: rgba(255,255,255,0.05);
  border: 1px solid var(--border-strong); border-radius: 100px; padding: 5px 8px 5px 12px;
}
.addon-count-chip {
  display: inline-flex; align-items: center; font-size: 11px; font-weight: 600;
  color: var(--accent); background: rgba(164,224,75,0.08);
  border: 1px solid rgba(164,224,75,0.2); border-radius: 100px; padding: 4px 10px;
}
.chip-remove {
  display: flex; align-items: center; justify-content: center;
